...
export function attachGridReveal(box, img, delay = 0, hero = false, holdMs = 0) {
  ...
  const asp = (() => { const w = parseFloat(img.getAttribute('width') || ''),
      h = parseFloat(img.getAttribute('height') || ''); return w > 0 && h > 0 ? w / h : 0.75; })();
  const t = fx();
  ...
  const s = { W: 0,
    ...
    sharp: null,
    now: 0,
    t0: performance.now() + Math.max(0, delay),
    holdMs: Math.max(0, holdMs),
    photoFrom: t.photoFrom,
    colorMs: t.colorMs,
    morph: t.morph,
    sheen: t.sheen,
    waitCap: t.waitCap,
  ...
  const makeBuffers = () => {
    ...
    x.drawImage(img,
      (s.W - img.naturalWidth * sc) / 2,
      (s.H - img.naturalHeight * sc) / 2,
      img.naturalWidth * sc,
