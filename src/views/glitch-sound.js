...
export { TYPES };
export function setEnabled(v) { on = !!v; const c = getCtx(); if (!c) return; if (on) { if (c.state === 'suspended') c.resume().catch(() => {}); if (!timer && el) schedule(); } else { if (c.state === 'running') c.suspend().catch(() => {}); clearTimeout(timer); timer = null; if (phase) { clearTimeout(phase); phase = null; } } }
