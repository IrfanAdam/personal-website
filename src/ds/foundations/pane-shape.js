...
export function html() {
  ...
  return `<div class="ds-sec"><h2>Shape — zero, normatively</h2><p class="sub">Every radius token resolves to <span class="tok">0</span>. Consume via <span class="tok">var(--radius-*)</span>; lint rejects literals. Toggle theme — values hold <span class="tok">0</span> in both.</p>`
    ...
    + focusSpec
    + note('Do',
      'Reach for borders and hairlines for structure — never a radius literal. Chamfer via <span class="tok">clip-path</span> when you need erosion (overview moodboard), not via <span class="tok">border-radius</span>.')
    + note('Don’t',
      'No <span class="tok">border-radius: 8px</span> / <span class="tok">999px</span> anywhere — zero is the geometry. The 8px card in the proof is the only literal in the DS and it exists to show what we don’t do.',
      'dont')
    + `</div>`;
}
