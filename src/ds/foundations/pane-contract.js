...
const index = [['Color', '--color-* + stone ramp → semantic → usage', 'bg, text, hairlines, accent'],
  ...
  ['Space · Layout', '--space-* + --size-* + --measure-* + --break-*', 'rhythm/gutter, wrap 1600, sm 640 / md 800 / lg 900'],
  ['Shape', '--radius-* (= 0, normatively) + --border-* + --color-focus', 'every radius + hairline/frame/dashed + focus ring'],
...
export function html() {
  ...
  + `</table>`
  + note('Do',
    'Add a token for every new value. Odd px (3/5/7/9/11) gets an explicit slot — never round silently. Convention: <span class="tok">--space-N</span> = raw step · <span class="tok">--size-*</span> = named object · <span class="tok">--measure-*</span> = ch · <span class="tok">--rhythm/--gutter</span> = purpose aliases (prefer) · <span class="tok">--break-*</span> = viewport (640/800/900 — see Space · Layout).')
  + note('Don’t',
  ...
  + note('Do',
    'New one-component need → new <span class="tok">--&lt;component&gt;-*</span> token. Forbid new semantic sprawl where a component token fits.')
  + note('Don’t',
    'No component token consumed by two components — shared need graduates to semantic, never duplicates per component.',
    'dont')
