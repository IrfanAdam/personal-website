...
export function renderSoundPanel() {
  ...
  html += [
    `<p class="sub" data-snd-desc style="margin:4px 0 0;font-size:12px;color:var(--color-ink-muted)">`,
    TYPE_META.hum,
    `</p>`,
  ].join('');
  ...
  html += [
    `<label class="fx-row">pitch <select data-snd="pitch"><option value="0">voice default</option>`,
    `<option value="1200">low</option><option value="2100">tick</option><option value="3400">high</option></select>`,
    `</label>`,
  ].join('');
  html += [
    `<label class="fx-row">length <input type="range" min="40" max="900" step="10" value="320" data-snd="len">`,
    `<output data-snd-v>320ms</output></label>`,
  ].join('');
  html += [
    `<label class="fx-row" data-snd-level-row style="display:none">level <input type="range" min="20" max="100" `,
    `step="5" value="100" data-snd="level"><output data-snd-g>100%</output></label>`,
  ].join('');
  html += `<p class="fx-status" data-snd-status>proximity on — hum level = distance to card</p>`;
  html += [
    `<p class="sub" style="margin-top:8px">Attr API: <span class="tok">data-glitch-sound="5000-9000 hum"</span> · `,
    `<span class="tok">data-glitch-sound="static,blip"</span> pools rotate per hit. Site hum is always proximity — `,
    `this toggle is lab preview only.</p></div>`,
  ].join('');
  return html;
}
