...
## Atlas — `#/atlas` (admin graph)

`src/ds/pages-atlas.js` is the route shell + interaction wiring; the canvas lives
in `src/ds/atlas/*` (shell → stage markup, dock → chips/legend/find, view → paint,
camera → pan/zoom/fit, layout → layered/radial positions, schema → filters +
`pathBetween` BFS, insight → overlay state, actions → find/focus/path commands,
search, tooltip, panel). The dataset is `src/ds/arch-schema.json` — 268 modules
and 512 import edges parsed from real `src/**` imports, regenerated by
`npm run arch` (`scripts/map-graph.mjs`), which also writes `docs/graph.mmd`.

How to read the canvas: the dock chips filter the three canvas layers
(`function` 29 · `component` 93 · `foundation` 28) and the edge kinds, and switch
layout between `layered` and `radial`; `#atlasFind` jumps to a node (Enter steps
through matches, zooms 1.6 and pins); click a node to focus it, shift-click a
second node for the shortest `imports` chain (hops crossing non-canvas layers are
listed in the inspector, but only on-canvas segments are drawn). Emphasis is one
contract: `data-dim` 0/1/2 = ink / muted ring / ghost, while `data-hit` (search)
and `data-path` (import chain) always win over dim. Dense columns (row step < 15)
render as dots whose labels appear on hover or focus.
